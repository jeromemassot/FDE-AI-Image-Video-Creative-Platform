...
- **Checklists:** A sidebar provides checklists to guide the user through the creation process.

The application is structured with a main `App.tsx` component that orchestrates the `ImageEditor` and `VideoGenerator` components. The API calls to the Google Generative AI are handled in `lib/api.ts` using the `@google/genai` library.
...
The project is also containerized with a `Dockerfile` and uses Google Cloud Build for CI/CD, as defined in `cloudbuild.yaml`.

## Development Conventions

- **API Key:** The application requires a Google Generative AI API key. The key is stored in the browser's `localStorage`. The user can set the key in the settings modal.
